...
{/* <nav>
       <div className="hidden head h-10 bg-[#18D9E4] md:flex justify-between items-center px-10">
        <h1 className='font-Inter text-sm text-gray-700'>Contact Us: +91-9354795744</h1>
        <h1 className='font-Inter text-sm text-gray-700 '>For Queries: info@krinvi.com</h1>
       </div>
       <div className="body xl:h-20 xl:justify-between xl:items-center md:h-48 flex justify-between items-center px-6 shadow-2xl">
            <div className="logo flex items-center ">
             <div className="md:hidden flex">
             <MenuOutlinedIcon sx={{fontSize:"35px"}} onClick={handleclick}/>
             </div>
             <img src="logo.svg" alt="" className=' w-3/4 '/>
            </div>
            ...
            <div className="hidden nav-mid text-black md:flex md:flex-col xl:flex-row xl:gap-14 xl:items-center">
                   

                  <div className="dropdown hidden xl:hidden md:flex md:gap-6  md:pr-48">
                   <div className=" "> 
                   <button className=" focus:outline-none md:text-lg font-medium  truncate md:flex">Shop by Category <span>{ isDropdownOpen ? <KeyboardArrowUpOutlinedIcon onClick={toggleDropdown}/>:<KeyboardArrowDownOutlinedIcon onClick={toggleDropdown}/>}</span></button>
                    {isDropdownOpen && (
                        <div className="absolute mt-2 bg-white shadow-lg rounded-md w-40">
                            <a href="#" className="block px-4 py-2 text-black hover:bg-gray-200">Sawan Special</a>
                            <a href="#" className="block px-4 py-2 text-black hover:bg-gray-200">Anime</a>
                  ...
                  <div className="hidden social-icon md:flex xl:hidden flex-col gap-3  justify-end items-end  md:mr-8 ">
                     <Link to={"#"} className=' font-medium md:text-lg'><InstagramIcon/> Instagram</Link>
                     <Link to={"#"} className='font-medium md:text-lg'><FacebookRoundedIcon/> Facebook</Link>
                  </div>
                  <div className="hidden md:flex xl:hidden md:gap-36   ">
                  
                  <Link to={"#"} className=' font-medium md:text-lg '>New Arrivals</Link>
                  
                  </div>

                   <div className="dropdown md:hidden xl:block">
                    <button className=" focus:outline-none">Shop by Category { isDropdownOpen ? <KeyboardArrowUpOutlinedIcon onClick={toggleDropdown}/>:<KeyboardArrowDownOutlinedIcon onClick={toggleDropdown}/>}</button>
                     {isDropdownOpen && (
                         <div className="absolute mt-2 bg-white shadow-lg rounded-md w-40">
                             <a href="#" className="block px-4 py-2 text-black hover:bg-gray-200">Sawan Special</a>
                             <a href="#" className="block px-4 py-2 text-black hover:bg-gray-200">Anime</a>
                   ...
                   <Link to={"#"} className='md:hidden xl:block  font-medium text-base'>Mens</Link>
                   <Link to={"#"} className='md:hidden xl:block  font-medium text-base'>Womens</Link>
                   <Link to={"#"} className='md:hidden xl:block  font-medium text-base'>Kids</Link>
                   <Link to={"#"} className='md:hidden xl:block  font-medium text-base'>New Arrivals</Link>
                   <Link to={"#"} className='md:hidden xl:block  font-medium text-base'><InstagramIcon/> Instagram</Link>
                   <Link to={"#"} className='md:hidden xl:block font-medium text-base'><FacebookRoundedIcon/> Facebook</Link>
            </div>
           <div className="nav-last flex  items-center gap-2">
           <div className="hidden md:block">
             <Badge badgeContent={0} showZero sx={{'& .MuiBadge-badge': {backgroundColor: 'black',color: 'white',}, }}>
                <ShoppingCartOutlinedIcon />
             </Badge>
